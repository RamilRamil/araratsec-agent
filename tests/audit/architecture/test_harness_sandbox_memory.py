...
import sr_agent.orchestrator.loop as _kernel_loop
from scripts.poc_queue_runner import _harness_sandbox
from sr_agent.tools.sandbox import DockerSandbox

_ROOT = Path(__file__).resolve().parents[3]
# The secure-agent surface: the audit pipeline (local) + the kernel loop it runs on.
# feature 048: the kernel is now an installed dependency — resolve loop.py via its
# module file rather than a local path (it no longer lives under this repo).
_SECURE_AGENT_FILES = [
    _ROOT / "audit_agent" / "pipeline.py",
    Path(_kernel_loop.__file__),
]
