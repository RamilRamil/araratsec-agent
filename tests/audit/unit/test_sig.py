...
from audit_agent.planner.sig import (
    build_sig,
    extract_state_vars,
    get_filtered_pairs,
)
...
def test_on_example_vault():
    root = Path(__file__).resolve().parents[3] / "examples" / "vulnerable-vault"
...
def _sg_graph():
    import json
    from pathlib import Path
    fx = Path(__file__).resolve().parents[2] / "fixtures" / "smartgraphical" / "sample_graph.json"
    return json.load(fx.open())


def test_sg_sig_reads_state_write_edge():
    from audit_agent.planner.sig import build_sig_from_smartgraphical
...
def test_regex_sig_misses_what_sg_catches():
    ...
    from pathlib import Path
    from audit_agent.planner.sig import build_sig
    root = Path(__file__).resolve().parents[3] / "examples" / "inheritance-vault"
