...
def test_qualified_name_falls_back_to_bare(fixture_project):
    """Live H-01 run (2026-07-05): the model asked `ICooldownVault.TGuard`
...
def test_shared_modifier_no_collision(fixture_project):
    ...
    cancel_impl = [m for m in cancel if m.contract == "CooldownVault"][0]
    finalize_impl = [m for m in finalize if m.contract == "CooldownVault"][0]
...
def test_top_level_symbols_gives_real_name_file_pairs(fixture_project):
    ...
    assert {"ICooldown", "CooldownVault"} <= names
