...
from audit_agent.analyze import analyze_target, build_analysis_prompt
from sr_agent.memory.episodic import EpisodicMemory
from audit_agent.session import AuditInput, AuditSession, Principal
from audit_agent.finding import Severity
...
def test_chat_detects_tool_calls(monkeypatch):
    ...
    lines = [
        ...
        b'[{"function": {"name": "lookup_symbol", "arguments": {"name": "TGuard"}}}]},'
        b' "done": true}\n',
    ]
    ...
    result = _lc.LocalClient(model="qwen3-coder:30b").chat(
        messages=[{"role": "user", "content": "what fields does TGuard have?"}],
        tools=[{"type": "function", "function": {"name": "lookup_symbol", "parameters": {}}}],
    )
    assert result["role"] == "assistant"
    assert result["tool_calls"] == [
        {"function": {"name": "lookup_symbol", "arguments": {"name": "TGuard"}}}
    ]
