...
"""
from audit_agent.proof.solidity_fixers import _fix_nested_type_imports
...
def test_nested_import_with_bare_use_is_rewritten():
    ...
    out, matched, applied = _fix_nested_type_imports(poc, _idx(), _FM)
...
def test_mixed_line_keeps_top_level_removes_nested():
    ...
    out, matched, applied = _fix_nested_type_imports(poc, idx, "I: i.sol")
    assert matched and applied
...
def test_top_level_only_import_unchanged():
    ...
    assert _fix_nested_type_imports(poc, idx, "") == (poc, False, False)


def test_library_import_untouched():
    poc = 'import { IERC20 } from "@openzeppelin/contracts/token/ERC20/IERC20.sol";\ncontract P {}'
    assert _fix_nested_type_imports(poc, _idx(), _FM) == (poc, False, False)


def test_aliased_import_left_unchanged():
    poc = 'import { S as Shares } from "../../contracts/I.sol";\ncontract P {}'
    out, matched, applied = _fix_nested_type_imports(poc, _idx(), _FM)
    assert matched is False and applied is False and out == poc


def test_idempotent():
    poc = ('import { I } from "i.sol";\nimport { S } from "i.sol";\n'
           'contract P { S s; }')
    out, _m, _a = _fix_nested_type_imports(poc, _idx(), _FM)
    out2, matched2, applied2 = _fix_nested_type_imports(out, _idx(), _FM)
    assert matched2 is False and applied2 is False and out2 == out
...
def test_none_index_is_noop():
    poc = 'import { S } from "i.sol";\ncontract P {}'
    assert _fix_nested_type_imports(poc, None, _FM) == (poc, False, False)
